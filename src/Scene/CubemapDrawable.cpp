...
void CubemapDrawable::draw(Camera* camera, D3D12_VIEWPORT& vp) {
	...
	for (int i = 0; i < 6; i++) {
		...
		// == IA ==
		cmdList->IASetVertexBuffers(0, 1, &quadVbv);
		cmdList->IASetIndexBuffer(&quadIbv);
		...
		cmdList->DrawIndexedInstanced(2 * 3, 1, 0, 0, 0);
	}
...
void CubemapDrawable::construct() {
	...
	//envMap->makeSrv(context, renderPipeline);
	ResourceHandle envCubeMapHandle = rm.createTexture(renderPipeline, 512, 512, {}, TextureType::ENV_MAP);
	...
	std::vector<Vertex> vertData{
		{XMFLOAT3(-1.f, -1.f, 0.f), XMFLOAT3(0.f, 0.f, -1.f), XMFLOAT2(0.f, 1.f)},
		{XMFLOAT3(-1.f,  1.f, 0.f), XMFLOAT3(0.f, 0.f, -1.f), XMFLOAT2(0.f, 0.f)},
		{XMFLOAT3(1.f,  1.f, 0.f), XMFLOAT3(0.f, 0.f, -1.f), XMFLOAT2(1.f, 0.f)},
		{XMFLOAT3(1.f, -1.f, 0.f), XMFLOAT3(0.f, 0.f, -1.f), XMFLOAT2(1.f, 1.f)}
	};
	...
	quadVb = rm.getVertexBuffer(vbHandle);
	...
	quadIb = rm.getIndexBuffer(ibHandle);

	quadVbv = quadVb->passVertexDataToGPU(*context, renderPipeline->getCommandList());
	quadIbv = quadIb->passIndexDataToGPU(*context, renderPipeline->getCommandList());
	...
	barriers[0].Transition.pResource = quadVb->getVertexBuffer().Get();
	...
	barriers[1].Transition.pResource = quadIb->getIndexBuffer().Get();
