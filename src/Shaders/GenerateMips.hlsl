...
float3 ApplySRGBCurve(float3 x)
{
    ...
    // used to be select, but not supported in this build
    float3 ret;
    ret.x = x.x < 0.0031308f ? 12.92f * x.x : 1.13005f * sqrt(abs(x.x - 0.00228f)) - 0.13448f * x.x + 0.005719f;
    ret.y = x.y < 0.0031308f ? 12.92f * x.y : 1.13005f * sqrt(abs(x.y - 0.00228f)) - 0.13448f * x.y + 0.005719f;
    ret.z = x.z < 0.0031308f ? 12.92f * x.z : 1.13005f * sqrt(abs(x.z - 0.00228f)) - 0.13448f * x.z + 0.005719f;
    return ret;
}
